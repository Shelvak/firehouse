window.Intervention ||=
  ...
  saveField: (field, callback, hold)->
    ...
    hash[fieldsetType] ||= {}
    hash[fieldsetType][fieldName] = fieldValue
    ...
    $.ajax
      ...
      success: (data)->
        ...
        if data.errors
          for fieldWithError, msgs in data.errors
            Intervention.colorizeAndShowMsgError(fieldset.find("name$=[#{fieldWithError}]"), msgs)
        else
          for k, v in data
            Intervention.cleanErrorsFromField(fieldset.find("name$=[#{fieldWithError}]"))
  ...
  colorizeAndShowMsgError: (field, msgs)->
    field.parents('.control-group:first').append($("<div class='alert alert-error'>#{msgs}</div>"))

  cleanErrorsFromField: (field)->
    field.parents('.control-group:first').append($("<div class='alert alert-error'>#{msgs}</div>"))
