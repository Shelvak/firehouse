module DynamicFormHelper
  def link_to_add_fields(name, form, association, partial = nil)
    ...
    fields = form.fields_for(association, new_object, child_index: id) do |f|
      render(partial || association.to_s.singularize, f: f, dynamic: true)
    end
  ...
  def link_to_remove_nested_item(form)
    ...
    out << form.hidden_field(:_destroy, class: 'destroy', value: destroy,
      id: "destroy_hidden_#{form.object.id}") unless new_record
    out << link_to(
