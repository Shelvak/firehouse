class MobileInterventionsController < ApplicationController
  ...
  def create
    @title = t('view.mobile_interventions.new_title')
    @mobile_intervention = @intervention.build_mobile_intervention(params[:mobile_intervention])

    respond_to do |format|
      if @mobile_intervention.save
        format.html { redirect_to [@intervention, @mobile_intervention], notice: t('view.mobile_interventions.correctly_created') }
        format.json { render json: [@intervention, @mobile_intervention], status: :created, location: @mobile_intervention }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @mobile_intervention.update_attributes(params[:mobile_intervention])
        format.html { redirect_to [@intervention, @mobile_intervention], notice: t('view.mobile_interventions.correctly_updated') }
        format.json { head :ok }
      else
    ...
    redirect_to edit_intervention_mobile_intervention(@intervention, @mobile_intervention), alert: t('view.mobile_interventions.stale_object_error')
  end
